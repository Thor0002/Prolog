...
kingpath1(Lpc,P,N) :- retractall(help1(_,_,_,_,_)),lpctolpn(Lpc,Lpn),unsafe(Lpn,Unsf),help(11,88,Unsf,P,N),N < 65.

help(V,T,_,[T],0) :- V = T,!.
help(V,T,Unsf,P,K) :- help1(V,T,Unsf,Ph1,Kh1) -> (P = Ph1, K = Kh1); 
                      ( Unsf1=[V|Unsf],
                          findall(Nv,move(V,Nv,Unsf1),Lnv),
                         (Lnv \= [] -> ( helpn(Lnv,T,Unsf1,P1,K1),P = [V|P1],K is K1 + 1,
                                         asserta((help1(V1,T,_,P,K) :- V1 = V, !)) );
                                       (P = [], K = 65) ) ).                                     
helpn([],_,_,[],65).
helpn([V1|L1],T,Unsf,P,K) :-helpn(L1,T,Unsf,P1,K1),help(V1,T,Unsf,P2,K2),
                           (K1 > K2 -> (P = P2,K = K2);(P = P1,K = K1) ).
